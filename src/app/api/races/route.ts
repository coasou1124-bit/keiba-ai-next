...
function buildRaces(rawRaces: RawRace[], learningModel: LearningModel): Race[] {
  return rawRaces.map(raw => {
    ...
    const horses = scoredHorses.map(h => {
...
export async function GET() {
  try {
    const [importedCount, allBets] = await Promise.all([
    ...
    const races = buildRaces(rawRaces, learningModel)
